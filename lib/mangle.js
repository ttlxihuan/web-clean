...
function mangleCssIdentifiers(identifiers, ignoreError) {
    ...
    function praseValue(source, standardValue) {
        ...
        if (varUseReg.test(source)) {
            var remove = false;
            var value = source.replace(varUseReg, function (str, _value, _, _default) {
                var name = identifiers('var-use', _value);
                if (typeof (name) === 'string') {
                    var newValue = 'var(--' + name;
                    if (_default !== undefined) {
                        var newDefault = parse(_default.replace(/^[,\s]+|[\s+]+$/g, ''));
