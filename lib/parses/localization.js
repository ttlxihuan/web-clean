...
function download(url, filename) {
    ...
    return new Promise((resolve, reject) => {
        ...
        function fail(msg) {
            clearTimeout(timer);
            console.error('[download] fail ' + msg + "\n" + url + ' => ' + filename);
            reject();
        }
        var timer = setTimeout(function () {
            console.error('[download] timeout ' + url + ' => ' + filename);
            req.abort();
            reject();
        }, 30 * 1000);
        const req = getProtocol().get(url, (response) => {
            ...
            if (response.statusCode !== 200) {
                console.error('[download] status: ' + response.statusCode + ' ' + url + ' => ' + filename);
                reject();
                return;
            }
...
function createLocalization(webOptions) {
    ...
    function mangleResourceFileIdentifier(pathname) {
        if (webOptions.mangleLocalize) {
            ...
            if (!resourceFileIdentifiers[_ext]) {
                resourceFileIdentifiers[_ext] = configConvertIdentifier(webOptions, 'mangleLocalize');
                resourceFileIdentifiers[_ext].reset('abcdefghijklmnopqrstuvwxyz0123456789_');
            }
