...
const { addslashes } = require('../identifier');
...
function mangleCssIdentifiers(identifiers, ignoreError) {
    ...
    function praseSelector(selector) {
        // 转义16进制码逆向
        selector = selector.replace(/(?!\\)*(\\{2})*\\([0-9a-fA-F]{1,6}) ?/g, function (_, $1, $2) {
            return ($1 === undefined ? '' : $1) + '\\' + String.fromCharCode(parseInt($2, 16));
        });
        var index = 0;
            ...
            while (index < selector.length) {
                            ...
                            if (/^:[\w\-]+\s*\(/.test(_selector)) {
                                ...
                                if (/^:(has|is|where|not|host)$/i.test(match[0])) {
                                    ...
                                    if (/^\s*\)/.test(selector.substr(index))) {
                                        ...
                                        throw new Error('Pseudo-Selector syntax error: ' + selector + "\n Unable to parse: " + _selector);
                                    }
                                } else {
                ...
                if (_selector !== null) {
                    ...
                    if (ignoreError) {
                        ...
                        console.info('Selector ignores: ' + ignore);
                        index += ignore.length;
                        continue;
                    }
                    throw new Error('Selector syntax error: ' + selector + "\n Unable to parse: " + _selector);
                }
            }
            if (sentence.length) {
                structure.push(sentence);
    ...
    function minifierSelector(selector, minifier) {
        function join(structure) {
            ...
            structure.forEach(items => {
                var selector = '';
                for (var str, key = 0; key < items.length; key++) {
                    if (typeof (items[key]) === 'string') {
                        if (/^:(not)$/i.test(items[key]) && key + 3 <= items.length - 1 && items[key + 1] === '(' && typeof (items[key + 2]) === 'object' && items[key + 3] === ')') {
                            str = join(items[key + 2]);
                            str = ':not(' + (str ? str : '\\!') + ')';
                            key += 3;
                        } else {
                            str = minifier(items[key]);
                        }
                    } else {
                        str = join(items[key]);
                    }
        ...
        for (var key = 0; key < tokens.length; key++) {
            ...
            switch (token[0]) {
                case Token.RULE:
                    ...
                    if (token[1].length <= 0) { // 选择器为空，不需要再处理
                        break;
                    }
...
function mangleHtmlIdentifiers(identifiers, removeAttr, sortAttributes = null) {
    var classAndIdReg = /((\t([a-z0-9]{1,11})\d+\3\t|[^\s\\]+|\\.)+)/ig;
    function replaceClassOrId(type, value, regexp, prefix = '') {
        return value.replace(regexp, function (source, str) {
            if (/\t([a-z0-9]{1,11})\d+\1\t/i.test(str)) { // 有占位符，自行跳过
                return source;
            }
            return prefix + unslashes(identifiers(type, addslashes(str)) || '');
        });
    }
    ...
    function htmlMinifier(tag, attrs) {
        ...
        for (var key = 0; key < attrs.length; key++) {
            ...
            switch (attrs[key].name.toLocaleLowerCase()) {
                case 'class':
                    value = replaceClassOrId('class', value, classAndIdReg)
                    break;
                case 'id':
                    value = replaceClassOrId('id', value, classAndIdReg)
                    break;
                case 'for':
                    if (/^label$/i.test(tag)) {
                        value = replaceClassOrId(['id-use', 'id'], value, classAndIdReg)
                    }
                    break;
                case 'href':
                    if (/^a$/i.test(tag) && /#.+/.test(value)) {
                        value = replaceClassOrId(['id-use', 'id'], value, /(?:#)([^?#]+)/g, '#');
                    }
                default:
