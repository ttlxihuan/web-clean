...
function WebClean(webOptions = {}, templateName = null) {
    ...
    async function eachPaths(callback, localization) {
        ...
        async function dispatch(pathname, sourceDir, targetDir, downloadUrl = null) {
            ...
            let targetFile = targetDir + _pathname;
            // 这里需要防止死循环复制
            if (strorage_files[targetFile] !== undefined) {
                return;
            }
            strorage_files[targetFile] = 1;
            if (ignore.test(_pathname)) {
                return;
            }
            let isSkip = skip.test(_pathname);
            try {
                for (var key in fileRules) {
                    if (fileRules[key].test(_pathname)) {
                        await callback(key, _pathname, sourceDir, targetDir, isSkip, downloadUrl);
                        return;
                    }
                }
                await callback('other', _pathname, sourceDir, targetDir, isSkip, downloadUrl);
            } catch (e) {
                console.error('[error-file] ' + sourceDir + _pathname);
                throw e;
            }
        }
        for (var next, file, iterator, key = 0; key < paths.length; key++) {
            let { sourceDir, targetDir } = paths[key];
            ...
            while (!(next = iterator.next(), file = next.value, next.done)) {
                ...
                await dispatch(pathname, sourceDir, targetDir, webOptions.baseUrl);
        ...
        for (let key = 0; key < localization.resourceFileMappings.length; key++) {
            ...
            await dispatch(pathname, downloadDir, targetDir, downloadUrl || webOptions.baseUrl);
            if (webOptions.localize) {
                await localization.waitAllDownload();
            }
        }
