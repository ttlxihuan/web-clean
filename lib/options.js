...
// const { jsMinifyOutput } = require('terser/output');
...
function makeOptions(options = {}) {
    ...
    // html选项处理
    let removeUnusedAttr = typeof (options.removeUnusedHtmlAttrs) === 'function' ? options.removeUnusedHtmlAttrs : function (name, value) {
        if (options.removeUnusedHtmlAttrs instanceof RegExp) {
            return options.removeUnusedHtmlAttrs.test(name);
        }
        return options.removeUnusedHtmlAttrs.indexOf(name) >= 0;
    };
    options.htmlOptions.sortAttributes = mangleHtmlIdentifiers(identifiers, removeUnusedAttr, options.htmlOptions.sortAttributes || null);
